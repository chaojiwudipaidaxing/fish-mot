...
LATEX_PDF_SRC="tmp/main.sanitized.pdf"
...
compile_latex_pdf() {
  local tex_input="$ROOT_DIR/tmp/main.sanitized.tex"
  local tmp_dir="$ROOT_DIR/tmp"

  mkdir -p "$tmp_dir/tables"
  cp paper/cea_draft/tables/*.tex "$tmp_dir/tables/"
  cp "$BIB_FILE" "$tmp_dir/references.bib"

  if command -v latexmk >/dev/null 2>&1; then
    if latexmk -pdf -interaction=nonstopmode -halt-on-error -output-directory="$tmp_dir" "$tex_input" \
      | tee "logs/latexmk.log"; then
      return 0
    fi
    echo "latexmk failed; falling back to manual pdflatex/bibtex passes." | tee -a "logs/latexmk.log"
  else
    echo "latexmk is not installed; using manual pdflatex/bibtex passes." | tee "logs/latexmk.log"
  fi

  pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$tmp_dir" "$tex_input" \
    | tee "logs/pdflatex_pass1.log"
  (cd "$tmp_dir" && bibtex main.sanitized) | tee "logs/bibtex.log"
  pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$tmp_dir" "$tex_input" \
    | tee "logs/pdflatex_pass2.log"
  pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$tmp_dir" "$tex_input" \
    | tee "logs/pdflatex_pass3.log"
}
...
if [[ -f "$LATEX_PDF_SRC" ]]; then
  cp "$LATEX_PDF_SRC" "$OUTDIR/CEA_manuscript_EN.pdf"
  echo "Copied LaTeX typeset PDF to $OUTDIR/CEA_manuscript_EN.pdf"
fi

if command -v soffice >/dev/null 2>&1; then
  if [[ ! -f "$OUTDIR/CEA_manuscript_EN.pdf" ]]; then
